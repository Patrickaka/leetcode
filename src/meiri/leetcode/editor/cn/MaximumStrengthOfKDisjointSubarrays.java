...
// 
//
// Related Topics 数组 动态规划 前缀和 👍 8 👎 0

package meiri.leetcode.editor.cn;

public class MaximumStrengthOfKDisjointSubarrays {
    public static void main(String[] args) {
        Solution solution = new MaximumStrengthOfKDisjointSubarrays().new Solution();
    }
    ...
    class Solution {
        public long maximumStrength(int[] nums, int k) {
            int n = nums.length;
            /*
             * dp数组 dp[i][j] 为 从nums[0] - nums[j - 1]中， 选出i个不相交的数组
             * 对于j : 不选j dp[i][j] = dp[i][j - 1]
             *          选j dp[i][j] = dp[i - 1][L] + (sums(j) - sums(L) * w
             *                       = sums[j] * w + dp[i - 1][L] - sums(L) * w
             */
            ...
            for (int i = 1; i <= k; i++) {
                ...
                for (int j = i; j <= n - k + i; j++) {
                    mx = Math.max(mx, f[i - 1][j - 1] - sums[j - 1] * w);
                    f[i][j] = Math.max(f[i][j - 1], sums[j] * w + mx);
                }
            }
            return f[k][n];
        }
