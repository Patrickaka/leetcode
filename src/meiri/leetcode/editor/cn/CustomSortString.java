...
// 
//
// Related Topics 哈希表 字符串 排序 👍 192 👎 0
    ...
    class Solution {
        public String customSortString(String order, String s) {
            ...
            for (int i = 0; i < s.length(); i++) {
                int cnt = sMap.getOrDefault(s.charAt(i), 0);
                sMap.put(s.charAt(i), cnt + 1);
            }
            StringBuilder sb = new StringBuilder();
            for (char c : order.toCharArray()) {
                if (sMap.containsKey(c)) {
                    sb.append(String.valueOf(c).repeat(Math.max(0, sMap.get(c))));
                    sMap.remove(c);
                }
            }
            for (Map.Entry entry : sMap.entrySet()) {
                sb.append(String.valueOf(entry.getKey()).repeat(Math.max(0, (Integer) entry.getValue())));
